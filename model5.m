...
obs_level2 =  3; % from 3 Possible sequence: 
...
D2{3} = [ones(states_lev2,1)]; % Report: {Up, Down, Baseline} %NO SÉ SI HAY PRIOR 1 acá % SEGUN CORTESE SÍ
...
A2{2} = zeros(obs_level2, states_lev2, T_2 ,  Ns2(3) );%
...
% r_vec1= r1'; 
% r2 = rand(1, (obs_level2-2)); % Start with N random numbers that don't sum to 1.
% r2 = r2 / sum(r2); % Normalize 
% r_vec2= r2'; 
% 
% A2{1}(3:end, 3, 1) = r_vec1; 
% A2{1}(3:end, 3, 2) = r_vec2; 
...
for i = 1:T_2
    for j = 3 % IT IS Baseline
        A2{2}(:,:,i,j) = [1/3 1/3 0 ;%Up
                          1/3 1/3 0 ; %Down
                          1/3 1/3 1 ];% Baseline
    end 
end
...
% 2.- TIME: Deterministically transition through trial sequence
B2{2} = zeros(Ns2(2),Ns2(2));

B2{2} = [0 0 0 ;
         1 0 0 ;
         0 1 1]; % Deterministically transition through trial sequence
...
B2{3} = zeros(Ns2(1), Ns2(1), action2_s1); 
...
%%% Report: "Up" (left column) , "Down" (central column), or "Baseline" (right column)

V2(2,:,3)= [1 2 3];



% %%% REPORT according to time step
% V2(:,1,1) =1;
% V2(:,2,2) =1;
% V2(:,3,3) =1;
% 
...
% % Stimuli
C2{1} = zeros( obs_level2, T_2-1);
% C2{1}(1,:) = pref2;
% C2{1}(2,:) = - pref2;
% C2{1}(3:end,:) = - pref2/2;

% % % % Observations
% C2{2} = zeros( obs_level2, T_2-1);
% C2{1}(1,:) = pref2;
% C2{1}(2,:) = - pref2;
% C2{1}(3:end,:) = - pref2/2;
...
C2{2}(1,end) = pref2; % no feedback yet
C2{2}(2,end) = - pref2; % preference not to be incorrect at last timestep
C2{2}(3,end) = - pref2/2;% preference for being correct at last timestep
