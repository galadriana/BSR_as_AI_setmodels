...
%%%% 
% Here the transitions between the Top-down states can be controlled 
...
U(:,:,2) = [u2_1  u2_2]; % HERE THE POLICY AIMS TO SWITCH FIRST LEVEL STATES
...
% Prior preferences over the policies - E map
% %--------------------------------------------------------------------------
...
label.factor{1}   = 'Bottom-up';   label.name{1}    = {'Up', 'Down'};
label.modality{1} = 'Thermometer';  label.outcome{1} = {'Up', 'Down'};
...
pathname = cd ;
filename = ( [ 'MDP_model8_50hs_' hoydia '_' num2str(time) '.mat'  ] ) ;
